(ns restaurant-management.actor
  "RestaurantManagementActor — the ISCO-08 1412 independent restaurant-
  management actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  restaurant operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the RestaurantManagementAdvisor can
  never directly commit a record or dispatch a robot action the
  RestaurantManagementGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [restaurant-management.advisor :as advisor]
            [restaurant-management.governor :as governor]
            [restaurant-management.store :as store]))

(defn build-graph
  "Build a compiled RestaurantManagementActor graph. `store`
  implements `restaurant-management.store/Store`. `advisor` implements
  `restaurant-management.advisor/Advisor` (defaults to
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:location-id (:location-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
